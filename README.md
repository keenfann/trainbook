...
- In-workout per-set rep and next-target weight updates that save back to the routine when the workout ends
...
- Analytics for overview, progression, interactive weekly/monthly trend charts (with regression + moving-average overlays), KPI cards including average days between workouts (7d/30d), drill-down volume/frequency distribution by muscle -> exercise, and bodyweight trend, including routine-type filtering (`standard` / `rehab` / `all`)
- Workout-duration KPIs ignore per-session durations above 180 minutes so accidentally unended sessions do not skew time-based stats
...
`APP_RELEASED_AT` controls the Settings -> About release timestamp (ISO 8601 recommended). The app displays this in the viewer's local timezone; timezone-less values are interpreted as UTC. If omitted, the build timestamp is used.
